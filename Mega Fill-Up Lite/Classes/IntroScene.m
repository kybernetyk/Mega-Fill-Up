...
//		[innerScrollerBack setPosition: ccp(0,3)];
		...
		[bar1 setPosition:  ccp (SCREEN_WIDTH/2, SCREEN_HEIGHT-10)];
		
		minyx = [CCSprite spriteWithFile: @"minyx.png"];
		[minyx setAnchorPoint: ccp(0.5,0.0)];
		[minyx setPosition:  ccp (SCREEN_WIDTH/2, SCREEN_HEIGHT-102-0)];
		
		bar2 = [CCSprite spriteWithFile: @"bar.png"];
		[bar2 setPosition:  ccp (SCREEN_WIDTH/2, SCREEN_HEIGHT-102-0)];
...
-(void) tick: (ccTime) delta
{
	...
	if (pos.y >= SCREEN_HEIGHT - 10)
	{	
		bar1_factor = -1.0;
		int newz = 2;
		[self reorderChild: bar1 z: newz];
	}
	if (pos.y <= SCREEN_HEIGHT-102-00)
	{	
		bar1_factor = 1.0;
		int newz = 4;
		[self reorderChild: bar1 z: newz];
